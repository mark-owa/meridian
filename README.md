...
**Stack:** React · FastAPI · PostgreSQL · Redis/Celery · Chroma · OpenAI

Meridian is a full-stack application for document search, structured extraction,
and lead qualification.

**Portfolio note:** Meridian was already being developed locally before this public
portfolio repository was created. The Git history here begins with the later import
and maintenance work rather than the project's first implementation.
...
The screenshots below are generated by a GitHub Actions + Playwright workflow
against the real Docker Compose application: PostgreSQL, Redis, Chroma, FastAPI,
nginx, and the production React build.

![Meridian dashboard](docs/assets/demo/dashboard.png)

| Leads | Knowledge Base |
| --- | --- |
| ![Meridian leads](docs/assets/demo/leads.png) | ![Meridian knowledge base](docs/assets/demo/knowledge-base.png) |

![Meridian document intelligence](docs/assets/demo/document-intelligence.png)

The capture registers a disposable user and creates demo leads through the real API.
It intentionally does **not** fabricate RAG answers, extraction results, or model
scores. See the machine-readable
[capture evidence](docs/assets/demo/capture-evidence.json).
...
- SQLite is intended for development. The Compose path runs Alembic before the API
  starts, and existing databases require Alembic migrations for schema changes.
  Tests create disposable SQLite schemas explicitly in their fixtures.
